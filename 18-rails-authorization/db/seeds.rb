...
User.destroy_all
seed_items(items)
user1 = User.create(username: "Eric", age:18, password: "abc123")
user2 = User.create(username: "Hasibul", age:18, password: "abc123")

5.times do 
    WishlistItem.create(user: user1, item: Item.all.sample)
end

3.times do 
    WishlistItem.create(user: user2, item: Item.all.sample)
end
